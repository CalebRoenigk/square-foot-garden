...
function viewConstructor(view, fromSave) {
  switch(true) {
      case view=='bed-layout':
        ...
        // Craft side panel
        document.querySelector('body').insertAdjacentHTML('afterbegin', '<div class="side-panel" data-state="side"><div class="container"><h1>Bed Layout</h1></div></div>');
        ...
        document.querySelector('.forward-action').insertAdjacentHTML('beforeend', '<div class="button-no-select button-forward">Select Plants<div class="button-arrow"></div></div>');

        // Add event listener to forward button to move to the next step
...
function transitionStep(stepFrom, direction) {
  let stepArray = ['bed-layout', 'plant-selection', 'analyze'];
  // Auto save the current data
  autoSaveData(stepFrom);

  // Determine which navigation path the user is taking
  if(direction == 'forward') {
    // Forward
    // Set the body step to the next step
    let newStep = stepArray[stepArray.indexOf(stepFrom)+1];
    document.querySelector('body').setAttribute('data-step', newStep);

    // Determine which transition to act on
    switch(true) {
      case stepFrom == 'bed-layout':
      // Go to plant-selection
      // Transition elements
      gsap.timeline()
        .to('.side-panel > .container > h1', {duration: .4, delay: .125, ease: 'power3.in', x: '-150%'})
        .to('.side-panel > .container > h1', {duration: .25, ease: 'linear', opacity: 0}, "-=.3")
        .to('.button-forward', {duration: .1, ease: 'linear', opacity: 0}, "-=.3")
        .call(changeText, ['.side-panel > .container > h1', 0, 'Choose Plants'])
        .call(changeText, ['.button-forward', 0, 'Planner'])
        .call(changeText, ['.button-backward', 1, 'Bed Layout'])
        .set('.side-panel > .container > h1', {x: '150%'})
        .set('.button-forward', {x: '-25%'}, "-=0")
        .set('.button-backward', {x: '25%'}, "-=0")
        .to('.button-forward', {className:"button-no-select button-forward"}, "-=.2")
        .to('.button-backward', {className:"button-secondary button-backward"}, "-=.2")
        .to('.side-panel > .container > h1', {duration: .7, ease: 'power3.out', x: '0%'}, "-=.5")
        .to('.side-panel > .container > h1', {duration: .3, ease: 'linear', opacity: 1}, "-=.5")
        .to('.button-backward', {duration: .5, ease: 'power3.out', x: '0%'}, "-=.75")
        .to('.button-backward', {duration: .225, ease: 'linear', opacity: 1}, "-=.5")
        .to('.button-forward', {duration: .5, ease: 'power3.out', x: '0%'}, "-=.675")
        .to('.button-forward', {duration: .225, ease: 'linear', opacity: 1}, "-=.425")
        .call(staggerInside, ['.content-bed-item', '.container > .bed-slider', .75, 0], "-=1")
        .to('.content-bed-add', {duration: .3, ease: 'linear', opacity: 0}, "-=.95")
        .to('.content-bed-item > .container > .bed-label > .subtract-icon', {duration: .2, ease: 'linear', stagger: {grid: 'auto', from: "start", axis: "y", each: .0125}, opacity: 0}, "-=.9")
        .to('.content-bed-item > .container > .bed-label > input', {duration: .5, delay: .1, ease: 'power3.inOut', stagger: {grid: 'auto', from: "start", axis: "y", each: .02}, x: '-38px'}, "-=.9")
        .call(generateCells)
        .call(cellTransition)
        .call(lockInputs, ['.content-bed-item > .container > .bed-label > input'])
        .call(modifyElements, [], "+=0");

      // Replace, modify, and add elements to be able finish the transition
      function modifyElements() {
        ...
        document.querySelectorAll('.content-bed-item > .container').forEach(function(element, i) {
          ...
          let cellContentsWrapperHeight = element.childNodes[1].getBoundingClientRect().height + element.childNodes[2].getBoundingClientRect().height + 10;
          ...
          let columnCount = document.querySelector('#' + bedID + '_width').value;

          // Delete the slider children
          element.removeChild(element.childNodes[1]);
          element.removeChild(element.childNodes[1]);

          // Replace them with the new cell contents wrapper
          element.insertAdjacentHTML('beforeend', '<div class="bed-wrapper" id="' + bedID + '_plants" data-bed-id="' + bedID + '" style="height: ' + cellContentsWrapperHeight + 'px;"></div>');

          // Override the grid for the bed item
          element.style.gridTemplateRows = "[name] 2em [bed-contents] auto";

          // Add the blank items as cell references
          document.querySelectorAll('#' + bedID + '_cells > rect').forEach(function(cell) {
            ...
            document.querySelector('#' + bedID + '_plants').insertAdjacentHTML('beforeend', '<div class="blank-item" data-cell="' + cellNum + '" id="empty_' + cellBed + '_' + cellNum + '" style="transform: scale(.25); opacity: 0;"></div>');
          ...
          gsap.to('#' + bedID + '_plants', {duration: elementDuration, delay: (elementOffsetDuration*i), ease: 'power2.inOut', height: newHeight});
          gsap.to('#' + bedID + '_plants > .blank-item', {duration: .45, ease: 'power2.out', stagger: {grid: 'auto', from: "start", axis: "y", amount: .5}, scale: 1})
          gsap.to('#' + bedID + '_plants > .blank-item', {duration: .225, ease: 'power2.out', stagger: {grid: 'auto', from: "start", axis: "y", amount: .5}, opacity: .2});
        });

        // Panel Content modification
        gsap.to('.panel-content', {duration: .25, ease: 'power1.inOut', paddingBottom: '60px'});

        // Bed Addition removal
        document.querySelector('.content-bed-add').remove();
        ...
        gsap.set('.search-wrapper', {top: document.querySelector('.panel-content').offsetTop, left: document.querySelector('.panel-content').offsetLeft, width: document.querySelector('.panel-content').getBoundingClientRect().width-6})
        ...
        gsap.timeline()
          .to('.search-spacer', {duration: .5, delay: .075, ease: 'power1.inOut', height: '0px', marginBottom: searchHeight-10})
          .from('.search-wrapper > .container', {duration: .625, ease: 'power3.out', x: '-50%'}, "-=.25")
          .to('.search-wrapper', {duration: .25, ease: 'linear', opacity: 1}, "-=.6")
          .set('.search-wrapper', {overflow: 'visible'})
  ...
  function staggerInside(parentSelector, childSelector, totalTime, timelinePosition) {
    ...
    document.querySelectorAll(parentSelector).forEach(function(parent, i) {
      ...
      gsap.to('#' + parent.id + ' > ' + childSelector, {duration: thisDuration, delay: thisDelay, ease: 'linear', stagger: {grid: [2,1], from: "start", axis: "y", amount: thisStaggerAmount}, opacity: 0});
    })
  }
...
function autoSaveData(stepOn) {
  ...
  gsap.timeline()
    ...
    .to('.autosave-wrapper', {duration: .625, ease: 'power2.inOut', scale: .9}, "+=.75")
