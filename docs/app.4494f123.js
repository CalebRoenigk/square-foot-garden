...
viewConstructor('bed-layout', false); // Build a view from scratch
...
function viewConstructor(view, fromSave) {
  switch (true) {
    case view == 'bed-layout':
      ...
      if (fromSave == true) {// Save state load
      } else {
        // Add side panel content
        document.querySelector('.side-panel').insertAdjacentHTML('beforeend', '<div class="panel-content"><div class="content-bed-add"><div class="add-icon"></div><h3>Add Bed</h3></div></div>'); // Add UI listeners
      ...
      gsap.timeline().to('.side-panel', {
        duration: 1,
        ease: 'power4.out',
        left: '40px'
      }).to('.file-button[data-file="load"]', {
        duration: .15,
        ease: 'linear',
        opacity: 1
      }, "-=.4").to('.file-button[data-file="load"]', {
        duration: .375,
        ease: 'circ',
        scale: 1
      }, "-=.45").to('.file-button[data-file="save"]', {
        duration: .15,
        ease: 'linear',
        opacity: 1
      }, "-=.3").to('.file-button[data-file="save"]', {
        duration: .375,
        ease: 'circ',
        scale: 1
      }, "-=.35").to('.button-forward', {
        duration: .25,
        ease: 'linear',
        opacity: 1
      }, "-=.25").to('.button-forward', {
        duration: .5,
        ease: 'circ',
        x: '0%'
      }, "-=.25").set('.file-button', {
...
function bedAddition() {
  ...
  function addBed(bedID, bedCount) {
    ...
    shadow.back(); // Scroll to the new bed's location
  ...
  function updateViewBounds() {
    ...
    var gridGroup = (0, _svg.SVG)('#grid-floor'); // Create a bounds object to store the min/max values
