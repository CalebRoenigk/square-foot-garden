...
function bedAddition() {
    ...
    document.querySelectorAll('div[data-bed-id="bed_' + bedID + '"] input[type="range"]').forEach(function (e) {
      e.addEventListener('input', function (r) {
        ...
        if (this.getAttribute('data-label') == 'Height') {
          (0, _svg.SVG)('#bed_' + bedID).height(this.value * 100);
        } else {
          (0, _svg.SVG)('#bed_' + bedID).width(this.value * 100);
        }

        redrawShadow(bedID, (0, _svg.SVG)('#bed_' + bedID));
      }, true);
  ...
  function updateViewportRender() {
    // Re-render the grid
    renderGrid(draw, 100); // This function renders the viewport grid
    ...
    function renderGrid(svgObject, cellSize) {
      ...
      let gridGroup = draw.group().attr({
        'data-type': 'grid-floor'
      }); // Iterate over the width, placing lines vertically

      for (var i = 0; i < viewportWidth / cellSize + 4; i++) {
        let line = svgObject.line((i - 2) * cellSize, -cellSize * 2, (i - 2) * cellSize, viewportHeight + cellSize * 2).stroke({
          width: 1,
          color: '#d8dadd',
          opacity: 1
        }).attr({
      ...
      for (var i = 0; i < viewportHeight / cellSize + 4; i++) {
        let line = svgObject.line(-cellSize * 2, (i - 2) * cellSize, viewportWidth + cellSize * 2, (i - 2) * cellSize).stroke({
          width: 1,
          color: '#d8dadd',
          opacity: 1
        }).attr({
...
//# sourceMappingURL=app.5e77f3c6.js.map