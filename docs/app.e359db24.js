...
function populatePanel(state) {
  switch (true) {
    case state == 'plant-selection':
      ...
      document.querySelector('.panel-content').insertAdjacentHTML('afterbegin', '<div class="search-wrapper"><div class="container"><div class="plant-search"><div class="search-box"><input type="text" placeholder="Start Typing" id="search-plants"><div class="search-suggestions"></div></div><div class="add-option" id="remove-zone"></div></div><div class="plant-items" id="no_bed_plants"><div class="search-container-dropzone"></div></div></div></div>'); // Event Listeners for Search
      ...
      document.querySelector('#remove-zone').addEventListener('dragleave', handleDragLeave, false); // Animate the content in
      // gsap.timeline()
...
function addPlantOption(plantObj) {
  ...
  let newItem = '<div class="grab-item" data-label-color="' + labelColor + '" id="plant_' + newPlantCount + '" data-cell="-1" draggable="true">' + plantObj.plant_name + '</div>'; // Add the drag and drop event listeners to the new item
...
function dragEnd(ev) {
  ...
  if (targetData !== null) {
    ...
    if (targetData.place !== null && targetData.place !== '-1') {
      ...
      if (dragData.place !== null && dragData.place !== '-1') {// The drag item is also a cell reference
      } else {
        // The drag item is not a cell reference
        // Set the cell color to the drag item color
        // Determine the cell id of the target item
        let cellID = '#cell_' + targetData.container.split('_')[1] + '_' + targetData.place; // Set the cell fill
        ...
        document.querySelector('#' + targetData.id).setAttribute('id', dragData.id); // Remove all event listeners

        removeDragAndDrop('#' + dragData.id); // Add new event listeners

        addDragAndDrop('#' + dragData.id); // Delete the drag item
...
function addDragAndDrop(elements) {
  ...
  dragableItems.forEach(function (item) {
    ...
    item.addEventListener('dragend', dragEnd, false); // item.addEventListener('dragenter', handleDragIn, false);

    item.addEventListener('dragover', allowDrop, false); // item.addEventListener('dragleave', handleDragOut, false);
...
var autoSave = setTimeout(function () {
  saveState();
}, 5000); // This function saves the current progress
...
//# sourceMappingURL=app.e359db24.js.map